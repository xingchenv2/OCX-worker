...
> 新机器请用下方向导安装；已部署机器可用 `ocx-worker update` 或重跑 `install.sh` 升级。
...
| 升级要敲一堆命令                           | 多步 systemctl + curl         | `ocx-worker update` 或重跑 install   |
| 日常运维（看日志/重启/备份/卸载）          | 一堆 systemctl/journalctl     | `ocx-worker` 进菜单                  |
| WebSSH 依赖 Docker                         | 旧方案常见                    | 二进制版，无 Docker 依赖            |

## 发布产物

- 应用 JAR：**[`latest`](https://github.com/xingchenv2/OCX-worker/releases/tag/latest)** Release
- 安装器与 CLI：**[`installer-latest`](https://github.com/xingchenv2/OCX-worker/releases/tag/installer-latest)** Release（`install.sh`、`ocx-worker`）
...
- 部署 `/usr/local/bin/ocx-worker`

本机**多数不会**安装 `mysql` 客户端（`ensure_mysql_client` 失败时安装器会警告并继续），因此：

| 操作 | 做法 |
|------|------|
| 清除 TG 绑定 | **`sudo ocx-worker tg-clean`** 或菜单 **11）清除Tg绑定**（自动 `docker exec ocx-worker-mysql`，密码来自 yml） |
| 查库里的 TG 配置 | 优先用 `ocx-worker tg-clean` 前的列表；勿猜密码，以 yml 里 `spring.datasource.password` 为准 |
| 备份数据库 | 建议 `apt install -y default-mysql-client` 后 `ocx-worker backup`，或自行 `docker exec ocx-worker-mysql mysql …` |
...
```bash
ocx-worker                  # 进交互菜单
ocx-worker status           # 服务状态
ocx-worker start/stop/restart
ocx-worker logs             # 实时日志
ocx-worker config           # 改端口/数据库（含回滚；账号密码请到 Web 设置）
ocx-worker update           # 一键升级
ocx-worker backup           # 备份数据库 + 配置 + keys
ocx-worker restore <file>   # 从备份恢复
ocx-worker tg-clean         # 清除 Telegram 绑定（无 mysql 客户端时自动 docker exec ocx-worker-mysql）
ocx-worker version          # 查看版本
ocx-worker uninstall        # 卸载（每步都问，给后悔药）
```
...
| `/opt/ocx-worker/backups/`               | `ocx-worker backup` 输出目录   |
| `/etc/systemd/system/ocx-worker.service` | 主程序 systemd                |
| `/usr/local/bin/ocx-worker`               | 管理脚本                      |
...
**Q: 装了新版还能切回旧版 JAR 吗？**  
A: 能。`ocx-worker update` 会从 `latest` Release 拉取；升级失败会自动回滚到上一版 JAR。若要整包重装，可用 `ocx-worker uninstall` 时保留 `/opt/ocx-worker` 数据目录后再跑 `install.sh`。

**Q: 升级会丢数据吗？**  
A: 不会。升级模式只换 JAR，**完全不动 application.yml 和数据库**。后端启动时由 `DatabaseGuardService` 自动 ALTER 加新表/新字段，旧数据 100% 保留。

**Q: 后端代码升级了，能直接 install 吗？**  
A: 可以。新版 JAR 推到 `latest` Release 后，`ocx-worker update` 或重跑 `install.sh` 都会拉最新版本。

**Q: 改坏了 application.yml 怎么办？**  
A: `ocx-worker config` 修改时会自动备份并在启动失败时回滚。手动改的话可以从 `/opt/ocx-worker/application.yml.bak.*` 找历史版本。
...
3. 万不得已：`ocx-worker uninstall` 选保留数据，重新安装一次（数据保留，密码会回到 Setup 流程）
